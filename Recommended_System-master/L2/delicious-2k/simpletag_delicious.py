...
class SimpleTagBased():
    ...
    def loadData(self):
        ...
        print("数据加载完成\n")
        print(self.records[7])
        # print(self.records.keys())
    
    # 将数据集拆分为训练集和测试集
    def randomlySplitData(self,ratio):
        # random.seed(seed)
        self.train=dict()
        self.test=dict()
    ...
    def precisionAndRecall(self, N):
        ...
        for user,items in self.test.items():
            if user not in self.train:        #如果user不在训练集内，无法检验，跳过
                continue
            # 获取Top-N推荐列表
            rank=self.recommend(user,N)
            print(rank)
            for item,rui in rank:   #针对对推荐的每一个item商品
                if item in items:   #如果推荐的item商品在records记录里，则推荐命中
                    hit+=1
            h_recall+=len(items)    #recall=用户推荐总命中个数/用户总共给多少商品打了标签（个数）
            h_precision+=N          #precision=用户推荐总命中个数/一共推荐的商品个数
        ...
        for tag, wut in self.user_tags[user].items():
            ...
            for item, wti in self.tag_items[tag].items():
                ...
                if item not in recommend_items:
                    ...
                    recommend_items[item] += wut * wti
        return sorted(recommend_items.items(), key=operator.itemgetter(1), reverse=True)[0:N]       #因迭代对象是元组，返回一个元组的列表；itemgetter(1)代表按值排序
